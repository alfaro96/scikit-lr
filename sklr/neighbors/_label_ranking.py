...
class KNeighborsLabelRanker(KNeighborsMixin,
                            LabelRankerMixin,
                            BaseNeighbors):
    """:term:`Label ranker` implementing the k-nearest neighbors vote.
    ...
    >>> X = [[0], [1], [2], [3]]
    >>> Y = [[1, 2, 3], [2, 3, 1], [3, 2, 1], [2, 3, 1]]
    >>> neigh = KNeighborsLabelRanker(n_neighbors=3)
    >>> neigh.fit(X, Y)
    KNeighborsLabelRanker(n_neighbors=3)
    >>> neigh.predict([[1.1]])
    array([[2, 3, 1]])
    """
    ...
    def fit(self, X, Y):
        """Fit the k-nearest neighbors :term:`label ranker` from
    ...
    def predict(self, X):
        """Predict the target :term:`rankings` for the provided
        data.
        ...
        Y : ndarray of shape (n_queries, n_classes)
            The target rankings for each data sample.
        """

...
cdef class Splitter:
    ...
    def __cinit__(self, Criterion criterion, INT64_t max_features,
        ...
        # the sorted indexes for the data
        self.X = None
        ...
        # Initialize the features indexes and the useful features
        self.features = None
    ...
    cdef void init(self, DTYPE_t_2D X, INT64_t_2D Y,
        ...
        # Initialize the features indexes
        self.features = np.zeros(self.n_features, dtype=np.intp)

        # Fill the features indexes and the useful features
        for feature in range(n_features):
            self.features[feature] = feature
            self.useful_features.push_back(feature)

        # Initialize the criterion from the rankings and the sample weight
        self.criterion.init_rankings(Y, sample_weight, self.max_splits)
    ...
    cdef void reset(self, SIZE_t_2D X_idx_sorted,
        ...
        # Reset the sorted indexes for the data
        self.X_idx_sorted = X_idx_sorted

        # Initialize the different attributes to be passed
        # to the criterion from the split record of the child
        ...
        # Reset the criterion from the record
        self.criterion.init_record(
    ...
    cdef void drawn(self):
        ...
        # Initialize the number of useful features
        n_useful_features = self.useful_features.size()

        # Obtain the random features
    ...
    cdef void node_split(self, SplitRecord split) nogil:
        ...
        # Obtain the best feature
        for feature in range(n_features):
            # Initialize the identifier of the feature
            ...
            # Obtain the best thresholds for the current feature
    ...
    cdef list node_indexes(self, SplitRecord record):
        ...
        # Initialize the view with the sorted indexes for the data
        # and the feature used for splitting
        X_idx_sorted = self.X_idx_sorted
        ...
        # Initialize the matrix where the indexes to keep will be stored
        matrix = <UINT8_t*> malloc(n_samples * sizeof(UINT8_t))

        # Initialize the new sorted indexes
        new_X_idx_sorted = list()

        # Initialize the arrays for the new sorted indexes
        for split in range(n_splits):
            new_X_idx_sorted.append(
                np.zeros((n_features, samples[split]), dtype=np.intp))

        # Obtain the sorted indexes for each split
        for split in range(n_splits):
            # For sanity, clear the contents of the indexes
            indexes.clear()
            # Copy the contents of the view to the set of indexes
            view_to_set_pointer_SIZE_1D(
                view=X_idx_sorted[best_feature, pos[split]:pos[split + 1]],
                pointer=&indexes)
            # Obtain the sorted indexes for the actual feature
            for feature in range(n_features):
                # Compute the boolean matrix
                in1d(X_idx_sorted[feature], indexes, &matrix)
                # Initialize the counter
                index = 0
                # Insert the elements into the memory view, checking the
                # contents against the computed boolean matrix
        ...
        # Return the new sorted indexes for the data
...
cdef class BinarySplitter(Splitter):
    ...
    cdef BOOL_t add_split(self, SIZE_t feature, SIZE_t sample) nogil:
        ...
        # Add a split at the very beginning
        return sample == 1
    ...
    cdef BOOL_t check_improvement(self, SIZE_t feature, SIZE_t sample,
        ...
        # Whenever the criterion is updated, check the improvement in impurity
...
cdef class FrequencySplitter(Splitter):
    ...
    cdef void init_params(self, SIZE_t feature) nogil:
        ...
        # Define some values to be employed
        cdef DTYPE_t first_sample_weight

        # Initialize the weight of the first sample
        first_sample_weight = self.criterion.sample_weight[
        ...
        # In fact, it is important to take into account that
        # substracting the weight of the first sample is needed
        # because it is skipped in the loop
        self.frequency -= first_sample_weight
    ...
    cdef BOOL_t update_criterion(self, SIZE_t feature, SIZE_t sample,
        ...
        # Update the criterion when a new split is added
        return add_split
    ...
    cdef BOOL_t check_improvement(self, SIZE_t feature, SIZE_t sample,
        ...
        """
        # Check the improvement in impurity on the last sample
...
cdef class WidthSplitter(Splitter):
    ...
    cdef void init_params(self, SIZE_t feature) nogil:
        ...
        # Initialize the feature value of the first sample and
        # and the feature value of the final sample
    ...
    cdef BOOL_t add_split(self, SIZE_t feature, SIZE_t sample) nogil:
        ...
        # Initialize the feature value of the first sample, of
        # the current sample, of the next sample and the final sample
        first_sample_value = self.X[
            self.X_idx_sorted[feature, 0],
            feature]
        prev_sample_value = self.X[
...
cdef class SplitRecord:
    ...
    def __cinit__(self, Criterion criterion,
        ...
        # Initialize the count or the precedences matrix on each split
    ...
    cdef void record(self, Criterion criterion, SIZE_t feature,
        ...
        # Initialize the counts or the precedences matrix
        if criterion.count_children is not None:
            self.counts[:] = (
        ...
        # Initialize the threshold value on each split
...
cdef void view_to_set_pointer_SIZE_1D(SIZE_t_1D view,
    ...
    # Copy the contents
    for sample in range(n_samples):
        deref(pointer).insert(view[sample])
...
cdef void in1d(SIZE_t_1D element, INDEXES_t test_elements,
    ...
    # Obtain the elements to keep
    for sample in range(n_samples):
        if test_elements.find(element[sample]) != test_elements.end():
            deref(matrix)[sample] = True
        else:
            deref(matrix)[sample] = False

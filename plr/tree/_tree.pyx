...
cdef class TreeBuilder:
    ...
    cdef void _build_leaf(self, Tree tree):
        ...
        # Initialize the number of samples and the weighted number of samples
    ...
    cdef void _build_internal(self, Tree tree, INT64_t depth):
        ...
        # Initialize the number of features and the number of maximum
        # splits from the stored values in the corresponding structures
        ...
        # Compute the new sorted indexes for the data
    ...
    cdef void _build(self, Tree tree, INT64_t depth=0):
        ...
        # Initialize the data, the rankings and the samples to consider
        X = self.splitter.X
        Y = self.splitter.criterion.Y
        samples = self.splitter.X_idx_sorted[0]

        # Initialize the pointer to the useful features
...
cdef class Tree:
    ...
    def __cinit__(self, INT64_t n_features, INT64_t n_classes):
        """Constructor."""
        # Initialize the number of features and the number of classes
        ...
        # Initialize the list of children of the tree
        self.children = list()
    ...
    cdef void _add_root(self, Node node):
        ...
        # Increase the number of internal nodes or leaves
    ...
    cdef void _add_child(self, Tree tree):
        ...
        # Increase the total number of internal nodes and leaves
    ...
    cpdef void _predict(self, DTYPE_t_1D x, INT64_t_1D prediction):
        ...
        # If the root of the tree is a leaf node, predict the consensus ranking
        if self.root.node == LEAF:
            prediction[:] = self.root.consensus
        # Otherwise, apply the decision path
        else:
...
cdef BOOL_t are_rankings_equal(INT64_t_2D Y, SIZE_t_1D samples) nogil:
    ...
    # Return whether all the rankings are equal
    return equal

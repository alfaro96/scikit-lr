...
from ._ranking_fast import (
    is_ranking_without_ties_fast, is_ranking_with_ties_fast, rank_view_fast)
...
def unique_rankings(Y):
    ...
    # Check whether the rankings are either for the Label
    # Ranking problem or the Partial Label Ranking problem
    if "unknown" in target_types:
        raise ValueError("Unknown label type: {}."
                         .format(sorted(list(target_types))))

    # Sort the rankings to find the unique ones in an efficient manner
...
def check_label_ranking_targets(Y):
    ...
    # Check whether the rankings are for Label Ranking
...
def check_partial_label_ranking_targets(Y):
    ...
    # Check whether the rankings are for Partial Label Ranking
...
def type_of_targets(Y):
    ...
    --------
    >>> import numpy as np
    >>> from plr.utils import type_of_targets
    ...
    # Transform the rankings to manage them in Cython
    ...
    # Label Ranking
    if all(map(lambda y: is_ranking_without_ties_fast(y), Y)):
        target_types.add("label_ranking")
    # Partial Label Ranking
    if all(map(lambda y: is_ranking_with_ties_fast(y), Y)):
...
def is_ranking_without_ties(y):
    ...
    --------
    >>> import numpy as np
    >>> from plr.utils import is_ranking_without_ties
...
def is_ranking_with_ties(y):
    ...
    --------
    >>> import numpy as np
    >>> from plr.utils import is_ranking_with_ties
...
def rank_data(data, method="ordinal", check_input=True):
    ...
    --------
    >>> import numpy as np
    >>> from plr.utils import rank_data

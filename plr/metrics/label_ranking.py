...
def _check_targets(Y_true, Y_pred):
    ...
    # Check that the input rankings are for Label Ranking
...
def kendall_distance(Y_true, Y_pred, normalize=True, sample_weight=None,
    ...
    --------
    >>> import numpy as np
    >>> from plr.metrics import kendall_distance
    ...
    # Return the mean Kendall distance according to the sample weights
    # (also return the array of distances if specified)
    if return_dists:
...
def tau_score(Y_true, Y_pred, sample_weight=None):
    ...
    --------
    >>> import numpy as np
    >>> from plr.metrics import tau_score
    >>> Y_true = np.array([[1, 2, 3], [3, 2, 1], [2, 1, 3]])
    >>> Y_pred = np.array([[2, 1, 3], [3, 2, 1], [1, 3, 2]])

...
cpdef void penalized_kendall_distance_fast(INT64_t_2D Y_true,
    ...
    # Compute the penalized Kendall distances
...
cdef DTYPE_t _penalized_kendall_distance_fast(INT64_t_1D y_true,
    ...
    # Initialize the penalized Kendall distance between the rankings to zero
    dist = 0.0
    ...
    for f_class in range(n_classes):
        for s_class in range(f_class + 1, n_classes):
            ...
            if (y_true[f_class] < y_true[s_class] and
                ...
                dist += 1
            # Penalized disagremeent if "f_class" precedes "s_class" in
            # one ranking and "f_class" is tied with "s_class" in the other
            # (or viceversa)
    ...
    # Normalize the penalized Kendall distance
    if normalize:
...
cdef DTYPE_t _tau_x_score_fast(INT64_t_1D y_true,
    ...
    # Initialize the Tau-x score between the rankings to zero
    score = 0.0
    ...
    for f_class in range(n_classes):
        for s_class in range(n_classes):
            ...
            # Also, if "f_class" precedes "s_class" in one ranking
            # and "f_class" is tied with "s_class" in the other (or viceversa)
            elif (y_true[f_class] < y_true[s_class] and
                    y_pred[f_class] < y_pred[s_class] or
                    y_true[f_class] < y_true[s_class] and
                    y_pred[f_class] == y_pred[s_class] or

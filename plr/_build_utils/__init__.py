...
def build_from_c_and_cpp_files(extensions):
    ...
    """
    # Obtain the source files when they correspond to a Cython extension module
    for extension in extensions:
        # Initialize the source files from
        # the current extension module
        sources = []
        # Check whether the source files correspond to a Cython module
        for source_file in extension.sources:
            ...
            if ext in {".pyx", ".py"}:
                if extension.language == "c++":
                    ...
                    ext = ".c"
                # Replace the current source file by the .c or .cpp one
...
def maybe_cythonize_extensions(top_path, config):
    ...
    # Otherwise, "cythonize" the .pyx files
        ...
        # Get the version needed to "cythonize" the extension modules
        cython_version = get_cython_version()
        # If it is not installed, show the corresponding message to the user

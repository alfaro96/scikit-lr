...
cdef class DistanceMetric:
        ...
        **kwargs :
        ...
        if isinstance(metric, type) and issubclass(metric, DistanceMetric):
            ...
            try:
                ...
                raise ValueError(
                    "Unrecognized metric: '{}'. Valid metrics are: {}."
                    .format(metric, sorted(list(METRIC_MAPPING.keys()))))

        # In Minkowski special cases, return more efficient methods
        if dist_metric is MinkowskiDistance:
            # Obtain the power parameter from the
            # keyworked arguments (default value 2)
            p = kwargs.pop("p", 2)
            # Manhattan distance
            if p == 1:
    ...
    cdef void cdist(self, DTYPE_t_2D X1, DTYPE_t_2D X2, DTYPE_t_2D dist) nogil:
        ...
        # Cmpute the cross-pairwise distances
    ...
    def pairwise(self, X1, X2=None):
        ...
        # distances between arrays X1 and X2
            ...
            # Ensure that X1 and X2 have the same second dimension
...
cdef class MinkowskiDistance(DistanceMetric):
    ...
    def __init__(self, p):
        """Constructor."""
        # Check the power hyperparameter

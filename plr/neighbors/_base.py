...
def _get_weights(Y, dist, weights):
    ...
    # Weight the samples according to the number of missing classes on each
    # nearest neighbor. This allow to obtain a more reliable prediction
    sample_weight *= np.mean(~np.isnan(Y), axis=2)

    # Weight the samples by the inverse of the distance
    if weights == "distance":
        # If user attempts to classify a point that was zero distance
        # from one or more training points, those training points are
        # weighted as 1.0 and the other points as 0.0
...
class BaseNeighbors(BaseEstimator, ABC):
    ...
    def _check_metric(self):
        ...
        # Obtain the metric object from the string identifier
    ...
    def fit(self, X, Y):
        ...
        # Check that if the metric is precomputed, then, X must be a squared
        ...
        if not isinstance(self.n_neighbors, (Integral, np.integer)):
            raise TypeError("n_neighbors does not take an integer value. Got {}."
        ...
        # Store the training instances (or distances) and the rankings
        (self._X, self._Y) = (X, Y)
    ...
    def _check_pairwise_arrays(self, X):
        ...
        (number of samples)."""
        # If the distances are precomputed, then, check that the second
        # dimension of the input array is equal to the number of
    ...
    def _pairwise_distances(self, X, reduce_func):
        ...
        # Check the input data to ensure that it is properly
        # formatted with respect to the training data
        self._check_pairwise_arrays(X)

        # Compute the distances between the input data and
        # the training input samples if they are not precomputed
        if self.metric != "precomputed":
...
class KNeighborsMixin:
    ...
    def _kneighbors_reduce_func(self, dist, n_neighbors, return_distance):
        ...
        # Initialize a sample range to extract the k-nearest neighbors
        sample_range = np.arange(dist.shape[0])[:, None]

        # Instead of using "np.argsort" to obtain the indices
        # of the k-nearest neighbors, "np.argpartition" is
        # employed, since it is more efficient
        # when k << n_samples, which is the general case
        neigh_ind = np.argpartition(
            dist, n_neighbors - 1, axis=1)[:, :n_neighbors]

        # In fact, "np.argpartition" does not
        ...
        # Return the nearest neighbors and the distances (if corresponds)
        return result
    ...
    def kneighbors(self, X=None, n_neighbors=None, return_distance=True):
        ...
        # If the distances of the training input samples must be computed,
        # store them into the corresponding variable and increase the number
        # of nearest neighbors to account for the sample itself being returned,
        ...
        # Obtain the number of samples of the input data
        n_samples = X.shape[0]

        # Compute the nearest neighbors

        # Create a variable with a partial function
        # that initializes the parameters for the
        # reduction function that transform distances
        ...
        # Once the result is know, it is important to remove the sample
        # itself being returned when the query is the training input data
        if query_is_train:
            # Check whether the distances are included in the result
            if return_distance:
            ...
            # than the number of neighbors, the first will not

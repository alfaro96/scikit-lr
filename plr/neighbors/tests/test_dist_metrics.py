...
# Local application
from plr.neighbors import DistanceMetric
...
X1 = random_state.random_sample((n_samples_X1, n_features)).astype(np.float64)
X2 = random_state.random_sample((n_samples_X2, n_features)).astype(np.float64)

# The different metrics to be tested
METRICS_HYPERPARAMS = {
    "euclidean": {},
    "chebyshev": {},
...
def test_pdist(metric):
    ...
    # Obtain the hyperparameters to be tested
    ...
    for hyperparams_values in product(*hyperparams.values()):
        ...
        # Check that the true distance and the predicted ones are the same

...
# Maximum random number for seeding
...
def _set_random_states(estimator, random_state=None):
    ...
    # Initialize the dictionary where the random states are going to be put on
    to_set = {}

    # Introduce the keys in the dictionary
    for key in sorted(estimator.get_hyperparams(deep=True)):
        if key == "random_state" or key.endswith("__random_state"):
            to_set[key] = random_state.randint(MAX_RAND_SEED)

    # Set the random states in the estimator
    if to_set:
...
class BaseEnsemble(MetaEstimatorMixin, BaseEstimator, ABC):
    ...
    def _make_estimator(self, append=True, random_state=None):
        ...
        # Include the estimator in the list of estimators
        if append:

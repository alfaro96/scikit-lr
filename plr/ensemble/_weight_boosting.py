...
This module contains weight boosting estimators for both Label Ranking and
...
- ``AdaBoostPartialLabelRanker`` implements adaptive boosting for
  Partiai Label Ranking problems.
"""
...
from ..metrics import penalized_kendall_distance as error_plr
...
class BaseWeightBoosting(BaseEnsemble, ABC):
    ...
    def fit(self, X, Y, sample_weight=None):
        ...
        # estimators an the errors of the estimators
        self.estimators_ = []
...
class AdaBoostLabelRanker(LabelRankerMixin, BaseWeightBoosting):
    ...
    --------
    >>> import numpy as np
    >>> from plr.ensemble import AdaBoostLabelRanker
    ...
    def _validate_estimator(self):
        ...
        # Call to the method of the parent using as
        # default estimator a Decision Tree Label Ranker
        super()._validate_estimator(
            default=DecisionTreeLabelRanker(max_depth=3))

        # Check that the estimator support sample weighting
...
# =============================================================================
# AdaBoost Partial Label Ranker
# =============================================================================
class AdaBoostPartialLabelRanker(PartialLabelRankerMixin, BaseWeightBoosting):
    """An AdaBoost Partial Label Ranker.

    AdaBoost [1] Partial Label Ranker is a meta-estimator that begins by
    fitting a Partial Label Ranker on the original dataset and then fits
    additional copies of the Partial Label Ranker on the same dataset
    but where the weights of incorrectly classified instances are adjusted
    such that subsequent Partial Label Rankers focus more on difficult cases.
    ...
    base_estimator : object, optional (default=None)
        The base estimator from which the boosted ensemble is built.
        Support for sample weighting is required. If ``None``, then
        the base estimator is ``DecisionTreePartialLabelRanker(max_depth=3)``.
    ...
    See also
    --------
    DecisionTreePartialLabelRanker, AdaBoostLabelRanker
    ...
    --------
    >>> import numpy as np
    >>> from plr.ensemble import AdaBoostPartialLabelRanker
    >>> X = np.array([[0, 1, 0], [1, 0, 1], [1, 1, 0], [0, 1, 0]])
    >>> Y = np.array([[1, 2, 3], [2, 1, 3], [1, 2, 3], [3, 1, 2]])
    >>> model = AdaBoostPartialLabelRanker(random_state=0)
    >>> clf = model.fit(X, Y)
    >>> clf.predict(np.array([[0, 1, 0]]))
    np.array([[1, 1, 1]])
    """
    ...
    def _validate_estimator(self):
        """Check the estimator and set the base_estimator_ attribute."""
        # Call to the method of the parent using as default
        # estimator a Decision Tree Partial Label Ranker
        super()._validate_estimator(
    ...
    def _boost(self, iboost, X, Y, sample_weight, random_state):
        """Implement a single boost for Partial Label Ranking."""
        # Make and fit the estimator
        estimator = self._make_estimator(random_state=random_state)
        estimator.fit(X, Y, sample_weight)
        ...
        # For each instance on the training dataset, obtain the error
        (estimator_error, instances_error) = error_plr(
            Y, Y_predict, sample_weight=sample_weight, return_dists=True)

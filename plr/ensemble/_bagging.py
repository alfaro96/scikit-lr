...
def _generate_indexes(random_state, bootstrap, n_population, n_samples):
    ...
    # Return the randomly drawn sample indexes
...
def _generate_bagging_indexes(random_state, bootstrap_features,
    ...
    # Return the randomly drawn features and samples
...
def _build_estimators(n_estimators, ensemble, X, Y, sample_weight, seeds):
    ...
    # Check whether the base estimator of the ensemble support sample weighting
    ...
    # Initialize the estimators and the features of the estimators
    estimators = []
    estimators_features = []
    ...
    for i in range(n_estimators):
        ...
        (features, indexes) = _generate_bagging_indexes(
            ...
            max_features, max_samples)
        # If the estimator support sample weights, draw
        # samples using sample weights and then fit
        if support_sample_weight:
            ...
            # Obtain the sample weights according to the bootstrap indexes
            if bootstrap:
        ...
        # Otherwise, directly fit the estimator
        else:
            estimator.fit(X[indexes][:, features], Y[indexes])
        # Append the built estimator to the list of estimators and the
        # features employed to build the estimator to the list of features
        estimators.append(estimator)
        estimators_features.append(features)

    # Return the built estimators and estimators features
...
class BaseBagging(BaseEnsemble, ABC):
    ...
    def _fit(self, X, Y, max_samples=None, sample_weight=None):
        ...
        # Check the maximum number of samples
    ...
    def fit(self, X, Y, sample_weight=None):
        ...
        self : object
        """
        # Call to the helper method to build the Bagging ensemble
...
class BaggingLabelRanker(LabelRankerMixin, BaseBagging):
    ...
    def _validate_estimator(self):
        ...
        # Call to the method of the parent with a
        # Decision Tree Label Ranking as base estimator
        super()._validate_estimator(
            default=DecisionTreeLabelRanker())
...
class BaggingPartialLabelRanker(PartialLabelRankerMixin, BaseBagging):
    ...
    --------
    >>> import numpy as np
    >>> from plr.ensemble import BaggingPartialLabelRanker
    >>> X = np.array([[0, 1, 0], [1, 0, 1], [1, 1, 0], [0, 1, 0]])
    >>> Y = np.array([[1, 2, 3], [2, 1, 3], [1, 2, 3], [3, 1, 2]])
    ...
    def _validate_estimator(self):
        ...
        super()._validate_estimator(
            default=DecisionTreePartialLabelRanker())
